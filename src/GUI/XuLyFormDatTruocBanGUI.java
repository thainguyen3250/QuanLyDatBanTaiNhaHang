...
public class XuLyFormDatTruocBanGUI extends JFrame implements ActionListener{
    public JTextField txtHoTen, txtSDT, txtMaRoomTable;
    ...
    public XuLyFormDatTruocBanGUI(TaiKhoan tk,Ban b) {
        super("Đặt Trước");
        ConnectDB.getInstance().connect();
        this.tk = tk;
        this.ban = b;
        band = new Ban_DAO();
        pdbd = new PhieuDatBan_DAO();
        ...
        title.add(editJLabel("Đơn Đặt Bàn Trước", 30));
        ...
        txtMaRoomTable.setText(ban.getMaBan());
        txtMaRoomTable.setEditable(false);
        row0.add(txtMaRoomTable);
        
        
        Box row05 = Box.createHorizontalBox();
        row05.add(editJLabel("Mã Phiếu Đặt Bàn: ", 20));
        ...
        try {
			txtMaPhieu.setText(pdbd.generateMaPhieuDatBan());
    ...
    public boolean xuLyDatTruoc()
    {
    	try
    	{
    		...
    		if (selectedDate == null) { 
    			JOptionPane.showMessageDialog(this, "Chưa Chọn Ngày Nhận Bàn!");
    			return false;
    		...
    		{
    			JOptionPane.showMessageDialog(this, "Chưa Nhập Đầy Đủ Thông Tin Khách Hàng!");
    			return false;
    		}
    		if(kh==null)
    		{
    		...
    		PhieuDatBan pdb = new PhieuDatBan(maPhieu, ngayDatBan, ngayNhanBan, ghiChu, diaChiQuan, soDienThoaiQuan, ban,tk.getMaNhanVien(),kh);
    		if(pdbd.themPhieuDatBan(pdb))
    		{
    			io.writeToFile(kh,"tempKhachHang/"+ban.getMaBan().trim());
    			band.setTrangThaiBan(ban.getMaBan().trim(), "Đã đặt trước");
	...
	public void actionPerformed(ActionEvent e) {
		...
		{
			if(JOptionPane.showConfirmDialog(this,"Bạn có muốn đặt trước bàn này không?","Cảnh Báo!",JOptionPane.YES_NO_OPTION)==JOptionPane.YES_OPTION)
			{
				if(xuLyDatTruoc()) {
					XuLyDatBanGUI.listBan.removeAll();
					XuLyDatBanGUI.napBanTheoKhuVuc(ban.getMaKhuVuc().getMaKhuVuc());
					XuLyDatBanGUI.listBan.revalidate();
					XuLyDatBanGUI.listBan.repaint();
					this.dispose();
				}
			}
		}
